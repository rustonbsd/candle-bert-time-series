...
/// Divergence-based trading strategy (LONG-ONLY for Binance API compatibility)
/// This strategy only takes long positions since shorts are not available via Binance API
...
impl DivergenceStrategy {
    ...
    /// Generate trading signals based on model divergence (LONG-ONLY STRATEGY)
    /// Since shorts are not available via Binance API, we use a long-only approach:
    /// - Negative divergence (model underestimated) -> BUY signal (expect catch-up)
    /// - Positive divergence (model overestimated) -> SELL existing positions (expect reversion)
    fn generate_signals(&self, data: &Tensor, timestamp: usize) -> Result<Vec<(usize, TradeSide, f64)>> {
        ...
        for (i, &crypto_idx) in self.analyzer.blacked_out_indices.iter().enumerate() {
            ...
            // Long-only trading strategy based on divergence
            if divergence.abs() > self.divergence_threshold {
                if divergence < 0.0 {
                    // Model underestimated -> expect catch-up -> BUY
                    signals.push((crypto_idx, TradeSide::Buy, self.position_size));
                } else {
                    // Model overestimated -> expect reversion -> SELL existing positions
                    // Note: This will only sell if we have existing positions in this crypto
                    signals.push((crypto_idx, TradeSide::Sell, self.position_size));
                }
            }
        }
...
fn main() -> Result<()> {
    ...
    println!("\n💰 DIVERGENCE-BASED TRADING STRATEGY (LONG-ONLY)");
    println!("======================================================================");
    println!("Note: This strategy only uses long positions due to Binance API limitations");
    ...
    println!("\n💡 STRATEGY INSIGHTS (LONG-ONLY):");
    println!("======================================================================");
    println!("This LONG-ONLY strategy trades on model divergence (no shorts due to Binance API):");
    println!("  - When model UNDERESTIMATES returns → BUY (expect catch-up)");
    println!("  - When model OVERESTIMATES returns → SELL existing positions (expect reversion)");
    println!("  - SELL decisions are position management, not short selling");
